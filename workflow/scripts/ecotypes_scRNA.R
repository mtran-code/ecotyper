...
if (is.na(min_states) || is.null(min_states) || min_states < 1) {
  warning(cat(
    "Config file field 'Minimum number of states in ecotypes' has an invalid value. Setting to the default value, 3.\n"
  ))
  min_states = 3
}
...
all_mapping$InitialID = paste(
  all_mapping$CellType,
  all_mapping$InitialState,
  sep = "_"
)
...
write.table(
  clusters,
  file.path(output_dir, "binary_classification_all_states.txt"),
  sep = "\t"
)
...
hclusCut <- function(x, k, ...) {
  list(cluster = cutree(hclust(as.dist(1 - x), method = "average", ...), k = k))
}
choose_clusters <- function(data, name, range = 2:10) {
  silh <- data.frame(
    K = range,
    Silhouette = sapply(range, function(k) {
  ...
  g2 <- ggplot(silh, aes(x = K, y = Silhouette)) +
    ...
    geom_line() +
    geom_vline(
      xintercept = silh[which.max(silh$Silhouette), 1],
      lty = 2,
      colour = "red"
    ) +
    theme_bw() +
  ...
  pdf(
    file.path(output_dir, paste0("nclusters_", name, ".pdf")),
    width = 4,
    height = 4
  )
  plot(g2)
  tmp = dev.off()
  png(
    file.path(output_dir, paste0("nclusters_", name, ".png")),
    width = 4,
    height = 4,
    res = 300,
    units = "in"
...
write.table(
  ...
  file.path(output_dir, "silhouette_initial.txt"),
  sep = "\t",
  row.names = F
)
...
write.table(
  top_ann,
  file.path(output_dir, "ecotypes.txt"),
  sep = "\t",
  row.names = F
)
...
diag(jaccard) = 1
pdf(
  file.path(output_dir, "initial_jaccard_matrix.pdf"),
  width = 7,
  height = 7,
  family = "Helvetica"
)
h <- heatmap_simple(
  ...
  top_columns = c("InitialEcotype", "Cell type", "State"),
...
if (length(tb) < 2) {
  warning(paste0(
    "There are less than 2 ecotypes with more than ",
    min_states,
    " cell states. Including ecotypes of size ",
    min_states - 1,
    ".\n"
  ))
...
pdf(
  file.path(output_dir, "jaccard_matrix.pdf"),
  width = 7,
  height = 7,
  family = "Helvetica"
)
h <- heatmap_simple(
  ...
  name = "ht1",
  top_annotation = top_ann,
  top_columns = c("Ecotype", "Cell type", "State"),
  legend_name = "Jaccard index",
  width = unit(2, "in"),
  height = unit(2, "in"),
  color_palette = c("gray", viridis(4)),
...
decorate_heatmap_body("ht1", {
  grid.rect(
    ...
    hjust = 0,
    vjust = 1,
    gp = gpar(col = "white", lty = 1, lwd = 2)
  )
})
tmp = dev.off()

png(
  file.path(output_dir, "jaccard_matrix.png"),
  width = 5,
  height = 7,
  res = 300,
  units = "in",
...
decorate_heatmap_body("ht1", {
  grid.rect(
    ...
    hjust = 0,
    vjust = 1,
    gp = gpar(col = "white", fill = NA, lty = 1, lwd = 2)
  )
})
tmp = dev.off()
