...
input_dir = file.path("../../data/procdata/datasets", dataset_type, dataset)
...
if (fractions == "Cell_type_specific_genes") {
  ...
  write.table(
    ...
    file.path(
      output_dir,
      paste0(cell_type, "_cell_type_specific_genes_raw.txt")
    ),
    sep = "\t",
    row.names = F
  )
  ...
  write.table(
    gene_info,
    file.path(output_dir, paste0(cell_type, "_cell_type_specific_genes.txt")),
    sep = "\t",
    row.names = F
  )
} else {
