...
if (!discovery %in% c("Carcinoma", "Lymphoma")) {
  discovery_config_file = file.path(
  ...
  if (!file.exists(discovery_config_file)) {
    stop(
      "Error: Cannot read the configuration file used for the discovery of cell states and ecotypes. It should be in the following path: '",
      config_file,
  ...
  if (
    !is.null(discovery_fractions) &&
      discovery_fractions %in% c("Carcinoma_Fractions", "Lymphoma_Fractions")
  ) {
...
PushToJobQueue(paste(
  "Rscript spatial_load_visium_data.R",
  recovery,
  paste0("'", input_path, "'")
))
RunJobQueue()
...
if (
  fractions %in%
    c("Carcinoma_Fractions", "Lymphoma_Fractions") &&
    !file.exists(fractions_path)
) {
  ...
  dir.create(
    file.path(
      "../../data/results/cibersortx/fractions/visium",
      recovery,
      fractions
    ),
    recursive = T,
...
for (cell_type in key[, 1]) {
  ...
  PushToJobQueue(
    (paste(
      "Rscript state_recovery_visium.R",
      discovery,
      fractions,
      cell_type,
      n_states,
...
PushToJobQueue(
  (paste("Rscript spatial_states.R", discovery, recovery, fractions, coo))
)
RunJobQueue()
cat("\nCalculating ecotype abundances...\n")
PushToJobQueue(
  (paste("Rscript spatial_ecotypes.R", discovery, recovery, fractions, coo))
)
RunJobQueue()
...
PushToJobQueue(
  (paste(
    "Rscript spatial_plot_ecotypes.R",
    discovery,
    recovery,
    fractions,
    coo
...
for (cell_type in key[, 1]) {
  system(paste0(
    ...
    file.path(
      ...
      "recovery",
      recovery,
      paste0(cell_type, "_spatial_heatmaps.pdf")
    ),
    "' '",
    final_output,
    "'"
  ...
  system(paste0(
    ...
    file.path(
      ...
      "recovery",
      recovery,
      paste0(cell_type, "_spatial_heatmaps.png")
    ),
    "' '",
    final_output,
    "'"
