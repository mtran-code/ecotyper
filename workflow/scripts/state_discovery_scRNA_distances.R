...
dataset_dir = file.path("../../data/procdata/datasets/discovery", dataset)
...
if (nrow(annotation) < 50) {
  stop(paste0(
    "Only ",
    nrow(annotation),
    " single cells are available for cell type: ",
    cell_type,
    ". At least 50 are required. Please remove this cell type from analysis and try again!\n"
  ))
}

if (nrow(annotation) > 2500) {
  warning(paste0(
    "There are more than 2500 single cells available for cell type '",
    cell_type,
    "'. ",
    "Subsampling to 2500 cells.\n"
  ))
  set.seed(1234)
...
if (filter_genes) {
  ...
  cat(paste0(
    "Not filtering '",
    cell_type,
    "' profiles for cell type specific genes, using the full transcriptome...\n"
  ))
}

write.table(
  scaled_data,
  file.path(output_dir, "expression_top_genes_scaled_filt.txt"),
  sep = "\t"
)
