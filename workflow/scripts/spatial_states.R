...
annotation = read.delim(file.path(
  "../../data/procdata/datasets/visium",
  recovery,
  "annotation.txt"
))

cell_fractions = read_fractions(file.path(
...
write.table(
  cell_fractions,
  file.path(output_dir, paste0(recovery, "_CSx_fractions.txt")),
  sep = "\t",
  row.names = F
)

if (
  is.null(malignant_cell) || is.na(malignant_cell) || malignant_cell == "NULL"
) {
  malignant_cell = "Placeholder"
} else {
  if (!malignant_cell %in% colnames(cell_fractions)) {
    warning(paste0(
      "Background cell type label, '",
      malignant_cell,
      "', not found amongst the cell populations analysed. Setting uniform gray background...\n"
    ))
    malignant_cell = "Placeholder"
  }
}
...
for (cell_type in key[, 1]) {
  ...
  tmp = cell_fractions[, c(
    colnames(cell_fractions)[1],
    cell_type,
    malignant_cell
  )]
  ...
  merged = merge(
    ...
    tmp,
    by.x = "row.names",
    by.y = colnames(cell_fractions)[1],
    all.x = T
  )
  ...
  melted$Abundance = melted$NMF_weighted_raw /
    quantile(melted$NMF_weighted_raw, .99, na.rm = T)
...
write.table(
  all_melted,
  file.path(output_dir, "state_abundances_long.txt"),
  sep = "\t",
  row.names = F
)
...
casted_data = dcast(
  all_melted,
  ID + X + Y + Sample + Malignant ~ State_ID,
  value.var = "Abundance"
)
write.table(
  casted_data,
  file.path(output_dir, "state_abundances.txt"),
  sep = "\t",
  row.names = F
)
