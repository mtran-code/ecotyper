...
csx_dir = file.path("../../data/results/cibersortx/hires", dataset, fractions)
exp_dir = file.path(
...
if (!file.exists(file.path(input_dir, "estim.RData"))) {
  stop(paste(
    "NMF output not found in '",
    file.path(input_dir, "estim.RData"),
    "'. Please make sure that cell state discovery ran successfully!"
  ))
}
#print(n_states)
...
gene_filt = apply(csx_data, 1, function(x) {
  rev(sort(table(unlist(x))))[1] > 0.5 * ncol(csx_data)
})
...
agg = apply(W, 2, function(x) {
  sum(x[grepl("_neg", rownames(W))]) / sum(x[grepl("_pos", rownames(W))])
})
...
write.table(
  tb,
  file.path(output_dir, "first_filter_data.txt"),
  sep = "\t",
  row.names = F
)
...
if (length(to_keep) < 2) {
  cat(paste0(
    ...
    cell_type,
    "', after filtering states with less than 10 marker genes! This indicates that the Cophenetic coefficient cutoff used at step 4 (choosing the number of cell states) might be set too high. You could try to lower it and re-run EcoTyper from step 4. Alternatively, the discovery dataset might be too small, or the cell type sepcific profiles contain too few genes. Not applying the filter for this cell type at the moment.\n"
  ))
...
if (length(intersect(to_keep2, to_keep)) < 2) {
  cat(paste0(
    "Warning: There would be less than two states left for '",
    cell_type,
    "', after applying the AFI filter! This indicates that the Cophenetic coefficient cutoff used at step 4 (choosing the number of cell states) might be set too high. You could try to lower it and re-run EcoTyper from step 4. Alternatively, the discovery dataset might be too small, or the cell type sepcific profiles contain too few genes. Not applying the filter for this cell type at the moment.\n"
  ))
} else {
