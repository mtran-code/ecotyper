...
if (annotation_path != "NULL" && columns != "NULL") {
  if (!file.exists(annotation_path)) {
    ...
    if (!"ID" %in% colnames(annotation)) {
      stop(
        "Error: The annotation file provided, does not contain the column 'ID'."
      )
    }
...
input_dir = file.path("data/procdata/datasets", "bulk", recovery)
