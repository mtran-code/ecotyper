...
if (!file.exists(barcode.path)) {
  stop(paste0(
    "File 'barcodes.tsv.gz' is missing from the input dir '",
    matrix_dir,
    "'"
  ))
}
if (!file.exists(features.path)) {
  stop(paste0(
    "File 'features.tsv.gz' is missing from the input dir '",
    matrix_dir,
    "'"
  ))
}
if (!file.exists(matrix.path)) {
  stop(paste0(
    "File 'matrix.mtx.gz' is missing from the input dir '",
    matrix_dir,
    "'"
  ))
}
if (!file.exists(positions.path)) {
  stop(paste0(
    "File 'tissue_positions_list.csv' is missing from the input dir '",
    matrix_dir,
    "'"
  ))
}

output_dir = file.path("../../data/procdata/datasets/visium/", id)
...
data = as.matrix(mat)
data = data.frame(
  Gene = feature.names[match(rownames(data), feature.names$V1), ]$V2,
  data
)
...
spatial_coords = spatial_coords[
  match(colnames(data)[-1], make.names(spatial_coords[, 1])),
]
