...
clinical$Ecotype = ifelse(
  (clinical$AssignmentQ < 0.25) & clinical$AssignedToEcotypeStates,
...
h <- heatmap_simple(
  ...
  color_range = seq(0, quantile(as.matrix(all_H), .9), length.out = 8),
...
h = heatmap_simple(
  ...
  height = unit(3, "in"),
  legend_name = "State abundance",
  color_range = c(seq(0, quantile(small_H, .8), length.out = 8)),
  color_palette = c("black", brewer.pal(8, "YlGnBu")),
  raster_quality = 20
)
