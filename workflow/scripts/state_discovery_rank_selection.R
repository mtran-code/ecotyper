...
find_threshold <- function(spl, min_cophenetic) {
  cross = c(
    F,
    F,
    sapply(3:nrow(spl), function(i) {
      spl[i, "Cophenetic"] < min_cophenetic &
        spl[i - 1, "Cophenetic"] >= min_cophenetic &
        spl[i - 2, "Cophenetic"] >= min_cophenetic
    })
  )
  if (length(which(cross)) == 0) {
    ...
    idx = last_cross -
      2 +
      which.min(c(
        abs(min_cophenetic - spl[last_cross - 1, "Cophenetic"]),
        abs(min_cophenetic - spl[last_cross, "Cophenetic"])
      ))
  }
...
biggest_drop <- function(spl) {
  ...
  cross = c(
    NA,
    sapply(2:nrow(spl), function(i) {
      spl[i - 1, "Cophenetic"] - spl[i, "Cophenetic"]
    })
  )
...
nclst = do.call(
  rbind,
  lapply(splits, function(spl) find_threshold(spl, min_cophenetic))
)
...
if (!all(!is.na(nclst$Chosen_Rank))) {
  ...
  cat(paste0(
    "************************************************\n",
    "WARNING: ",
    "The Cophenetic coefficient cutoff provided in the config file (",
    min_cophenetic,
    ") did not work for the following cell types: ",
    paste0(
      as.character(nclst[is.na(nclst$Chosen_Rank), "CellType"]),
      collapse = ", "
    ),
    " ",
...
write.table(
  nclst,
  file.path(output_dir, "rank_data.txt"),
  row.names = F,
  sep = "\t"
)
write.table(
  nclst,
  file.path(output_dir, paste0("rank_data_cutoff_", min_cophenetic, ".txt")),
  row.names = F,
  sep = "\t"
)
...
nclst$Alpha = factor(
  ifelse(nclst$Redefined, "", "Cophenetic cutoff supplied"),
  levels = c("", "Cophenetic cutoff supplied")
)
scale_alpha = c(0, 1)
...
pdf(
  file.path(output_dir, "rank_plot.pdf"),
  family = "Helvetica",
  useDingbats = F,
  width = 3 * sqrt(length(table(all_data$CellType))),
  height = 2.5 * sqrt(length(table(all_data$CellType)))
)
g <- ggplot(all_data, aes(x = n_clusters, y = Cophenetic)) +
  ...
  geom_vline(
    data = nclst,
    aes(
      xintercept = Chosen_Rank,
      col = "Number of states selected by EcoTyper"
    ),
    lty = 2
  ) +
  geom_hline(
    data = nclst,
    aes(yintercept = min_cophenetic, alpha = Alpha),
    col = "black",
    lty = 2
  ) +
  theme_publication() +
...
pdf(
  file.path(output_dir, paste0("rank_plot_cutoff_", min_cophenetic, ".pdf")),
  family = "Helvetica",
  useDingbats = F,
  width = 3 * sqrt(length(table(all_data$CellType))),
  height = 2.5 * sqrt(length(table(all_data$CellType)))
...
png(
  file.path(output_dir, "rank_plot.png"),
  family = "Helvetica",
  width = 3 * sqrt(length(table(all_data$CellType))),
  height = 2.5 * sqrt(length(table(all_data$CellType))),
  units = "in",
