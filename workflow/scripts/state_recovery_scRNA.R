...
if (ncol(data) > 2500 && subsample < 50) {
  warning(paste0(
    "More than 2500 cells are available for cell type '",
    cell_type,
    "'. State recovery might take a long time. We recommend using the option -s to downsample the data..."
  ))
}

if (subsample >= 50 && ncol(data) > subsample) {
  cat(paste0(
    "Downsampling cell type '",
    cell_type,
    "' to ",
    subsample,
    " cells...\n"
...
write.table(
  data,
  file.path(output_dir, "expression_matrix_scaled.txt"),
  sep = "\t"
)
...
write.table(
  H,
  file.path(output_dir, "initial_state_abundances.txt"),
  sep = "\t"
)

assignment = data.frame(
  ID = colnames(H),
  State = apply(H, 2, function(x) rownames(H)[which.max(x)])
)
assignment = assignment[order(assignment$State), ]
write.table(
  assignment,
  file.path(output_dir, "initial_state_assignment.txt"),
  sep = "\t",
  row.names = F
)
...
assignment$State = mapping[
  match(assignment$InitialState, mapping$InitialState),
  "State"
]
...
write.table(
  assignment,
  file.path(output_dir, "state_assignment.txt"),
  sep = "\t",
  row.names = F
)
...
sum_squares <- function(hmcs) {
  ...
  if (ncol(hmcs) > 100) {
    ...
    for (i in 1:nrow(hmcs)) {
      d <- sapply(c(1:(ncol(hmcs) - w)), function(j) {
        mean(hmcs[i, c(j:(j + w))], na.rm = T)
      })
      if (i == 1) {
        smooth <- d
      } else {
        smooth <- rbind(smooth, d)
      }
    }
    ...
    hmcs <- smooth
    hmcs <- (hmcs -
      sapply(1:nrow(hmcs), function(i) mean(hmcs[i, ], na.rm = T)))
    thres <- .005
...
suppressWarnings({
  discovery_data = fread(
    file.path(states_dir, "heatmap_data.txt"),
    data.table = F
  )
})
...
scRNA_plot_data = scRNA_plot_data[
  match(rownames(discovery_data), rownames(scRNA_plot_data)),
  match(assignment$ID, colnames(scRNA_plot_data))
]
...
assignment$State = factor(
  as.character(assignment$State),
  levels = levels(discovery_annotation$State)
)

if (
  is.null(smoothed_data) || nrow(smoothed_data) == 0 || ncol(smoothed_data) == 0
) {
  stop(paste(
    "No samples were assigned to cell states for cell type:",
    cell_type
  ))
}
...
suppressWarnings({
  p <- heatmap_simple(
    discovery_data,
    top_annotation = discovery_annotation,
    dataset = dataset,
    top_columns = unique(c(
      top_cols[top_cols %in% colnames(discovery_annotation)],
      "State"
    )),
  ...
  q <- heatmap_simple(
    smoothed_data,
    top_annotation = assignment,
    dataset = dataset,
    top_columns = unique(c(
      top_cols[top_cols %in% colnames(assignment)],
      "State"
    )),
    column_title = paste0(
      "Single cells assigned to cell states\nn = ",
      nrow(assignment),
      " (",
      format(nrow(assignment) * 100 / ncol(H), digits = 2),
      "%)"
    ),
    name = "hmap2",
    scale_rows = F,
...
pdf(
  file.path(output_dir, "state_assignment_heatmap.pdf"),
  width = 9,
  height = 6
)
suppressWarnings({
  draw(
    top_markers_left(scRNA_plot_data, 5, gene_info, "State") + p + q,
...
rect = rectangle_annotation_coordinates(
  gene_info$State,
  discovery_annotation$State
)
...
decorate_heatmap_body("hmap1", {
  grid.rect(
    ...
    y = unit(rect$y, "native"),
    width = unit(rect$w, "native"),
    height = unit(rect$h, "native"),
    hjust = 0,
    vjust = 1,
    gp = gpar(col = "white", lty = 1, lwd = 1)
  )
})
decorate_heatmap_body("hmap2", {
  grid.rect(
    x = unit(rect2$x, "native"),
    y = unit(rect2$y, "native"),
    width = unit(rect2$w, "native"),
    height = unit(rect2$h, "native"),
    hjust = 0,
    vjust = 1,
    gp = gpar(col = "white", lty = 1, lwd = 1)
  )
...
png(
  file.path(output_dir, "state_assignment_heatmap.png"),
  width = 9,
  height = 6,
  units = "in",
  res = 300
...
decorate_heatmap_body("hmap1", {
  grid.rect(
    ...
    hjust = 0,
    vjust = 1,
    gp = gpar(col = "white", fill = NA, lty = 1, lwd = 1)
  )
})
decorate_heatmap_body("hmap2", {
  grid.rect(
...
write.table(
  scRNA_plot_data,
  file.path(output_dir, "heatmap_data.txt"),
  sep = "\t"
)
write.table(
  assignment,
  file.path(output_dir, "heatmap_top_ann.txt"),
  sep = "\t"
)
...
initial_assignment = read.delim(file.path(
  output_dir,
  "initial_state_assignment.txt"
))
initial_gene_info = read.delim(file.path(states_dir, "initial_gene_info.txt"))
data = data[
  match(initial_gene_info$Gene, rownames(data)),
  match(initial_assignment$ID, colnames(data))
]
rownames(data) = initial_gene_info$Gene
data[is.na(data)] = 0

if (calculate_significance_flag) {
  calculate_recovery_significance(
    data,
    initial_gene_info,
    initial_assignment,
    output_dir,
