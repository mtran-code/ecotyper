...
input_mixture = fread(
  file.path("../../data/procdata/datasets/", dataset_type, dataset, "data.txt"),
  data.table = F
)
fractions_dir = file.path(
...
csx_fractions = csx_fractions[
  match(colnames(input_mixture)[-1], csx_fractions$Mixture),
]

write.table(
  csx_fractions,
  file.path(output_dir, "cibresults.txt"),
  sep = "\t",
  row.names = F
)
...
write.table(
  t(classes),
  file.path(output_dir, "classes.txt"),
  sep = "\t",
  quote = F,
  row.names = F,
  col.names = F
...
tmp = lapply(1:length(splits), function(x) {
  ...
  fwrite(
    splits[[x]],
    file.path(sub_dir, paste0("input.txt")),
    sep = "\t",
    row.names = F,
    quote = F
  )
  PushToJobQueue(paste(
    "Rscript csx_hires_worker.R",
    dataset,
    fractions,
    x,
    1,
