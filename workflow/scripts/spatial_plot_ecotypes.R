...
ecotype_data$Abundance = ecotype_data$Abundance *
  tb[match(as.character(ecotype_data$Ecotype), tb[, 1]), 2]
ecotype_data$Abundance = ecotype_data$Abundance /
  quantile(ecotype_data$Abundance, .99, na.rm = T)
...
spatial_heatmap_hexagon_confocal <- function(
  data,
  columns,
  colors,
  name = name,
  ...
  hmp = Heatmap(
    as.matrix(mat),
    name = paste0(paste(columns, collapse = "_"), name),
    cluster_rows = F,
    cluster_columns = F,
    na_col = "white",
    col = colorRamp2(breaks = c(0, 1), colors = c("white", "white")),
    show_row_names = F,
    show_column_names = F,
    ...
    cell_fun = function(jj, ii, x, y, w, h, col) {
      ...
      for (column in columns) {
        ...
        alpha = data[
          as.character(data$Y) == (colnames(mat)[jj]) &
            as.character(data$X) == (rownames(mat)[ii]),
          column
        ]
        ...
        grid.polygon(
          x = lx,
          y = ly,
          gp = gpar(col = col, fill = col, alpha = alpha, lwd = 0)
        )
      }
    },
    ...
...
pdf(
  file.path(output_dir, paste0("Ecotype", "_spatial_heatmaps.pdf")),
  height = 2.6 * nr + 1,
  width = nc * 2.3
)
pushViewport(viewport(
  layout = grid.layout(
    nc = 1,
    nr = 2,
    heights = c(unit(2.6 * nr, "in"), unit(0.25, "in"))
  )
))
...
for (spl in splits) {
  ...
  spatial_heatmap_hexagon_confocal(
    spl,
    columns,
    colors,
    name = title,
...
g <- Heatmap(
  as.matrix(c(legend_text, "Ecotype")),
  col = c(background, charcoal),
  name = "hmap",
  width = 0,
  height = 0,
...
png(
  file.path(output_dir, paste0("Ecotype", "_spatial_heatmaps.png")),
  height = 2.8 * nr + 1,
  width = nc * 2.3,
  units = "in",
  res = 300
