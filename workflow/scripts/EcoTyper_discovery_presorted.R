...
filter_genes = as.logical(
  config$"Pipeline settings"$"Filter non cell type specific genes"
)
...
if (config$"Pipeline settings"$"Filter genes" == "cell type specific") {
  ...
  if (config$"Pipeline settings"$"Filter genes" == "no filter") {
    ...
    if (is.na(n_genes)) {
      stop(paste0(
        "Invalid value provided in field 'Filter genes' of the configuration file."
      ))
    }
...
if (!1 %in% skip_steps & fractions != "All_genes") {
  ...
  for (cell_type in colnames(classes)) {
    PushToJobQueue(paste(
      "Rscript state_discovery_presorted_filter_genes.R",
      discovery,
      fractions,
      cell_type
    ))
...
if (!2 %in% skip_steps) {
  ...
  cat(
    "Step 2 (cell state discovery): Running NMF (Warning: This step might take a long time!)...\n"
  )
