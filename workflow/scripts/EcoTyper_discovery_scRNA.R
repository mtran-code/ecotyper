...
if (!1 %in% skip_steps & fractions != "All_genes") {
  ...
  for (cell_type in cell_types) {
    ...
    PushToJobQueue(paste(
      "Rscript state_discovery_scRNA_filter_genes.R",
      discovery,
      fractions,
      cell_type,
      scale_column
...
if (!4 %in% skip_steps) {
  ...
  for (cell_type in key[, 1]) {
    ...
    PushToJobQueue(paste(
      "Rscript state_discovery_initial_plots_scRNA.R",
      "discovery_cross_cor",
      discovery,
      fractions,
      cell_type,
...
if (!5 %in% skip_steps) {
  ...
  for (cell_type in key[, 1]) {
    cat(paste(
      "Extracting marker genes for cell states defined in:",
      cell_type,
      "\n"
    ))
    n_clusters = key[key[, 1] == cell_type, 2]
    PushToJobQueue(paste(
      "Rscript state_discovery_extract_features_scRNA.R",
      discovery,
      fractions,
      cell_type,
      n_clusters
...
if (!7 %in% skip_steps) {
  ...
  for (cell_type in key[, 1]) {
    ...
    n_clusters = key[key[, 1] == cell_type, 2]
    PushToJobQueue(paste(
      "Rscript state_discovery_first_filter_scRNA.R",
      discovery,
      fractions,
      cell_type,
      n_clusters,
...
if (!8 %in% skip_steps) {
  ...
  RunJobQueue()
  PushToJobQueue(paste(
    "Rscript ecotypes_assign_samples_scRNA.R",
    discovery,
    fractions,
    "Ecotype",
    paste(additional_columns, collapse = " ")
...
for (cell_type in key[, 1]) {
  ...
  #system(paste("cp -f", file.path("../../data/results/ecotyper_work", discovery, fractions, "Cell_States", "discovery", cell_type, n_clusters, "state_abundances.txt"), paste0("'", ct_output, "'")))
  system(paste(
    "cp -f",
    file.path(
      "../../data/results/ecotyper_work",
