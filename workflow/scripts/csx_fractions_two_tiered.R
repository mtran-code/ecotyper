...
sig = cbind(
  tmp_sig[,
    !(colnames(tmp_sig) %in%
      as.character(mappings[, 1]) |
      colnames(tmp_sig) %in% as.character(mappings[, 2]))
  ],
  sig
)
...
write.table(
  sig,
  file.path(output_dir, "CIBERSORTx_Results.txt"),
  sep = "\t",
  quote = F,
  row.names = F
)
