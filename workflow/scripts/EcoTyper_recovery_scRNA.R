suppressPackageStartupMessages({
  library(argparse)
  source("workflow/scripts/lib/config.R")
  source("workflow/scripts/lib/multithreading.R")
})
...
arguments$add_argument(
  ...
  default = "Carcinoma",
  help = paste0(
    "The name of the discovery dataset used to define cell states and ecotypes. Accepted values: ",
...
if (!discovery %in% c("Carcinoma", "Lymphoma")) {
  ...
  if (!file.exists(config_file)) {
    stop(
      "Error: Cannot read the config file used for the discovery of cell states and ecotypes. It should be in the following path: '",
      config_file,
      "'. Please make sure that the '--discovery (-d)' argument provided is correct!"
    )
  }
...
if (!file.exists(annotation_path)) {
  ...
  if (!all(c("ID", "CellType") %in% colnames(annotation))) {
    stop(
      "Error: The annotation file provided, does not contain the columns 'ID' and 'CellType."
    )
  }

  if ("Sample" %in% colnames(annotation)) {
    ecotypes = read.delim(file.path(
      "data/results/ecotyper_work",
      discovery,
      fractions,
      "Ecotypes",
    ...
    if (length(rec_cell_types) * 2 >= length(disc_cell_types)) {
      ...
      cat(
        "The annotation file contains column 'Sample', but less than half of the cell types are present in the recovery dataset. Will NOT perform ecotype recovery.\n"
      )
      recover_ecotypes = F
    }
  } else {
    recover_ecotypes = F
    cat(
      "The annotation file does not contain column 'Sample'. Will NOT perform ecotype recovery.\n"
    )
  }
  ...
  if (columns != "NULL") {
    ...
    if (!all(additional_columns %in% colnames(annotation))) {
      stop(paste0(
        "The following columns are missing from the annotation file provided: ",
        "'",
        paste(
          additional_columns[!additional_columns %in% colnames(annotation)],
          collapse = "'"
        ),
        "'"
      ))
...
input_dir = file.path("data/procdata/datasets", "scRNA", recovery)
...
system(paste0(
  ...
  annotation_path,
  "' '",
  file.path(input_dir, "annotation.txt"),
  "'"
))
...
if (nrow(key) == 0) {
  stop(
    "Error: The column 'CellType' of the annotation file contains invalid values!"
  )
}
for (cell_type in key[, 1]) {
  ...
  PushToJobQueue(
    (paste(
      "Rscript state_recovery_scRNA.R",
      discovery,
      fractions,
      cell_type,
      n_states,
...
if (recover_ecotypes) {
  PushToJobQueue(
    (paste("Rscript ecotypes_recovery_scRNA.R", discovery, fractions, recovery))
  )
  RunJobQueue()
}
...
if (file.exists(final_output) && length(list.files(final_output)) > 0) {
  old_results_folder = paste0(
    final_output,
    format(Sys.time(), " %a %b %d %X %Y")
  )
  dir.create(old_results_folder, recursive = T, showWarnings = F)
  warning(paste0(
    "The output folder contains files from a previous run. Moving those files to: '",
    old_results_folder,
    "'"
  ))
...
cat(paste0(
  "\nEcoTyper finished succesfully! Please find the results in: '",
  final_output,
  "'.\nRun time: ",
  format(end - start, digits = 1),
  "\n"
))
