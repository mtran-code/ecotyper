...
scaled_data = read.delim(file.path(
  input_dir,
  "expression_top_genes_scaled_filt.txt"
))
...
if (nrow(scaled_data) < 50) {
  stop(paste0(
    "Only ",
    nrow(scaled_data),
    " genes were imputed for cell type: ",
    cell_type,
    ". At least 50 are required. Skipping this cell type!"
  ))
}
...
write.table(
  FCs,
  file.path(output_dir, "full_FCs.txt"),
  sep = "\t",
  row.names = T
)
FCs = FCs[FCs$MaxFC > 0, ]
...
write.table(
  FCs,
  file.path(output_dir, "top_FCs.txt"),
  sep = "\t",
  row.names = T
)
