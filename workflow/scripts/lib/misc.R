...
abspath <- function(files) {
  file.path(normalizePath(dirname(files)), basename(files))
}
...
read_fractions <- function(input_dir) {
  if (file.exists(file.path(input_dir, "CIBERSORTx_Adjusted.txt"))) {
    ...
    if (file.exists(file.path(input_dir, "CIBERSORTx_Results.txt"))) {
      ...
      stop(paste0(
        "File ",
        file.path(input_dir, "CIBERSORTx_Adjusted.txt"),
        " does not exist! Please make sure you ran CIBERSORTx fractions!"
      ))
    }
  }
...
get_variable_genes <- function(m) {
  ...
  df$dispersion_norm <- with(
    df,
    abs(dispersion - bin_disp_median) / bin_disp_mad
  )
...
scale_data <- function(data, by = NULL) {
  ...
  data = reconstituted_data[, match(
    colnames(data),
    colnames(reconstituted_data)
  )]
  data
}
...
read_clinical <- function(
  ...
  if (
    ...
    cat(paste0(
      "No annotation file found for dataset: ",
      dataset,
      ". Creating empty annotation.\n"
    ))
    clinical = data.frame(ID = sample_list)
    return(clinical)
  }
  clinical = read.delim(file.path(
    "../../data/procdata/datasets",
    dataset_type,
    dataset,
    "annotation.txt"
  ...
  if (palette == 1) {
    ...
    if (palette == 2) {
      ...
      if (palette == 3) {
        cols = rep(
          c(
            colorRampPalette(brewer.pal(8, "Accent"))(8)[c(
              3,
              5,
              6,
              4,
            ...
            colorRampPalette(brewer.pal(8, "Pastel1"))(8)
          ),
          10000
        )
        cols[6] = "lightblue"
        ...
        if (palette == 4) {
          cols = rep(
            c(
              ...
              brewer.pal(8, "Dark2")[-c(5, 6, 7)],
              rev(brewer.pal(8, "Set2")),
            ...
            if (palette == 6) {
              ...
              if (palette == 7) {
                ...
                if (palette == 8) {
                  ...
                  cols = c(
                    rgb(214, 55, 46, maxColorValue = 255),
                    rgb(250, 221, 75, maxColorValue = 255),
                    rgb(112, 180, 96, maxColorValue = 255),
                    rgb(230, 144, 193, maxColorValue = 255),
                    rgb(152, 94, 168, maxColorValue = 255),
                    rgb(163, 163, 163, maxColorValue = 255),
                    rgb(183, 211, 229, maxColorValue = 255),
                    rgb(230, 216, 194, maxColorValue = 255),
                    rgb(240, 143, 53, maxColorValue = 255),
                    rgb(81, 137, 187, maxColorValue = 255)
                  )
                  ...
                  if (palette == 9) {
                    ...
                    cols = rep(
                      c(
                        colorRampPalette(brewer.pal(8, "Accent"))(limit),
                        colorRampPalette(brewer.pal(8, "Set2"))(limit),
                        colorRampPalette(brewer.pal(8, "Pastel1"))(limit)
                      ),
                      10000
                    )
                  }
...
collapse_samples <- function(data, variable) {
  agg_data = do.call(
    cbind,
    lapply(levels(variable), function(x) {
...
get_FC_for_clusters <- function(data, variable) {
  logFC_data = do.call(
    cbind,
    lapply(levels(as.factor(as.character(variable))), function(cluster) {
      clust_samples = variable == cluster
      clust_mean_exp = apply(data[, clust_samples, drop = F], 1, function(x) {
        mean(x, na.rm = T)
      })
      non_clust_mean_exp = apply(
        data[, !clust_samples, drop = F],
        1,
        function(x) {
          mean(x, na.rm = T)
        }
...
doDE <- function(data, variable, value = NULL) {
  ...
  logFC_data = do.call(
    ...
    lapply(vals, function(cluster) {
      ...
      data.frame(
        Gene = rownames(data),
        State = cluster,
        FC = clust_mean_exp - non_clust_mean_exp,
        P = p,
  ...
  logFC_data = logFC_data[
    order(logFC_data$State, -logFC_data$FC, logFC_data$P),
  ]
  logFC_data
}
...
lookup_celltype <- function(values) {
  unlist(lapply(as.character(values), function(x) {
    gsub(".", " ", gsub(".and.", "/", x, fixed = T), fixed = T)
  }))
}

lookup_celltype_short <- function(values) {
  prefix_length = 3:max(unlist(lapply(as.character(values), function(x) {
    nchar(x)
  })))
