...
averge_FC_marker_genes <- function(data, gene_info, assignment) {
  data = data[
    match(gene_info$Gene, rownames(data)),
    match(assignment$ID, colnames(data))
  ]
  FC = sapply(levels(as.factor(gene_info$State)), function(x) {
    if (sum(assignment$State == x) == 0) {
      return(rep(NA, nrow(data)))
    }
    apply(data[, assignment$State == x, drop = F], 1, function(y) {
      mean(y, na.rm = T)
    }) -
      apply(data[, assignment$State != x, drop = F], 1, function(y) {
        mean(y, na.rm = T)
...
calculate_recovery_significance <- function(
  data,
  initial_gene_info,
  initial_assignment,
  output_dir,
  ...
  ref_average = averge_FC_marker_genes(
    data,
    initial_gene_info,
    initial_assignment
  )
  ...
  cat(
    "Running recovery significance calculation. This step might take a long time..."
  )
  coef_null = NULL
  boot_averages = NULL
  ...
  lapply(1:n_bootstraps, function(x) {
    ...
    write.table(
      ...
      file.path(
        bootstrap_output,
        paste0("bootstrap_", x, "_expression_matrix.txt")
      ),
      sep = "\t"
    )
    ...
    save(
      nmf,
      file = file.path(bootstrap_output, paste0("bootstrap_", x, ".RData"))
    )
    coef <- apply(nmf@fit@H, 2, function(x) (x / sum(x)))
    assignment = data.frame(
      ID = colnames(rand_data),
      State = apply(coef, 2, function(x) colnames(W)[which.max(x)])
    )
    rownames(coef) = sprintf("IS%02d.B%02d", 1:n_states, x)
    coef_null <<- rbind(coef_null, coef)

    boot_average = averge_FC_marker_genes(
      rand_data,
      initial_gene_info,
      assignment
    )
  ...
  write.table(
    zs_corr,
    file.path(output_dir, "recovery_z_scores_initial_states.txt"),
    sep = "\t",
    row.names = F
  )
  ...
  write.table(
    filt_zs_corr,
    file.path(output_dir, "recovery_z_scores.txt"),
    sep = "\t",
    row.names = F
  )
}
