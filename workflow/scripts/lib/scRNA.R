...
read_scRNA <- function(dataset, cell_type, log2 = F, scale_rows = F) {
  ...
  if (nrow(annotation) == 0) {
    ...
    #	"'. Please make sure that column 'CellType' in file '", file.path("../../data/procdata/datasets/scRNA", dataset, "annotation.txt"), "' contains value '", cell_type, "'"))
    return(NULL)
  }

  expr = fread(
    file.path("../../data/procdata/datasets/scRNA", dataset, "data.txt"),
    data.table = F
  )
  ...
  if (nrow(annotation) == 0) {
    stop(paste0(
      "The cell IDs do not match between the column names of expression matrix and column 'CellType' of the annotation file, for cell type '",
      cell_type,
      "'."
    ))
  }
  ...
  if (log2) {
    if (!all(expr >= 0)) {
      stop(paste0(
        "Not all values in the expression matrix of dataset '",
        dataset,
        "' are positive. Could not take log2!"
      ))
    }
    expr = log2(expr + 1)
  }
