...
annotation = read_clinical(
  colnames(data),
  dataset = new_dataset,
  dataset_type = new_dataset_type
)
...
if (
  is.null(scRNA_plot_data) ||
    nrow(scRNA_plot_data) == 0 ||
    ncol(scRNA_plot_data) == 0
) {
  stop(paste(
...
suppressWarnings({
  ...
  q <- heatmap_simple(
    ...
    column_title = paste0(
      "Bulk samples assigned to cell states\nn = ",
      nrow(assignment),
      " (",
