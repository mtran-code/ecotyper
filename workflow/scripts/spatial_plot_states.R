...
pdf(
  file.path(output_dir, paste0(cell_type, "_spatial_heatmaps.pdf")),
  height = 2.6 * nr + 1,
  width = nc * 2.3
)
...
g <- Heatmap(
  as.matrix(c(legend_text, "Cell state")),
...
png(
  file.path(output_dir, paste0(cell_type, "_spatial_heatmaps.png")),
  height = 2.6 * nr + 1,
  width = nc * 2.3,
  units = "in",
  res = 300
