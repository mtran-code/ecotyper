...
for (n_clusters in 2:max_n_clusters) {
  ...
  for (restart in 1:max_restarts) {
    ...
    if (!file.exists(file.path(input_dir, "estim.RData"))) {
      if (verbose) {
        cat(paste0(
          "Warning: NMF output file '",
          file.path(input_dir, "estim.RData"),
          "' is missing! Skipping it...\n"
        ))
      }
      next
    }
  ...
  write.table(
    t(as.data.frame(used_reps)),
    file.path(output_dir, "used.txt"),
    sep = "\t"
  )
  ...
  write.table(
    all_coef,
    file.path(output_dir, paste0("rank_data.txt")),
    sep = "\t",
    row.names = F
  )
}
