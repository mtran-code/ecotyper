suppressPackageStartupMessages({
  ...
  source("workflow/scripts/lib/misc.R")
  ...
  if (fractions %in% c("Carcinoma_Fractions", "Lymphoma_Fractions")) {
    if (discovery_type == "RNA-seq") {
      PushToJobQueue(paste(
        ...
        CSx_username,
        CSx_token,
        paste0("'", CSx_singularity_path_fractions, "'")
      ))
      RunJobQueue()
      PushToJobQueue(paste(
        "Rscript csx_fractions.R",
      ...
      RunJobQueue()
      PushToJobQueue(paste(
        "Rscript csx_fractions_two_tiered.R",
        "discovery",
        discovery,
        "TR4",
        "no_batch",
    ...
    cat(
      "Step 1 (cell type fraction estimation): Loading user-provided cell type fractions...\n"
    )

    dir.create(
      file.path(
        "../../data/results/cibersortx/fractions/discovery",
        discovery,
        fractions
      ),
      recursive = T,
...
if (!2 %in% skip_steps) {
  ...
  PushToJobQueue(paste(
    ...
    CSx_token,
    n_threads,
    paste0("'", CSx_singularity_path_hires, "'")
  ))
  RunJobQueue()

  cat(
    "Step 2 (cell type expression purification): Aggregating CIBERSORTxHiRes results...\n"
  )
  PushToJobQueue(paste(
    "Rscript csx_hires_aggregate_worker_results.R",
    "discovery",
    discovery,
    fractions
  ))
...
if (!3 %in% skip_steps) {
  ...
  classes_path = file.path(file.path(
    "../../data/results/cibersortx/hires",
    discovery,
    fractions,
    "classes.txt"
  ))
  ...
  for (cell_type in colnames(classes)) {
    PushToJobQueue(paste(
      "Rscript state_discovery_extract_features.R",
      discovery,
      fractions,
      cell_type,
      scale_column
  ...
  cat(
    "Step 3 (cell state discovery): Running NMF (Warning: This step might take a long time!)...\n"
  )
  classes_path = file.path(file.path(
    "../../data/results/cibersortx/hires",
    discovery,
  ...
  for (cell_type in colnames(classes)) {
    ...
    PushToJobQueue(paste(
      "Rscript state_discovery_combine_NMF_restarts.R",
      "discovery",
      discovery,
      fractions,
      cell_type,
...
if (!4 %in% skip_steps) {
  ...
  PushToJobQueue(paste(
    "Rscript state_discovery_rank_selection.R",
    "discovery",
    discovery,
    fractions,
    max_clusters,
...
if (!5 %in% skip_steps) {
  ...
  system(paste(
    "cp -f ",
    paste0("'", config_file, "'"),
    file.path("../../data/results/ecotyper_work", discovery, "config_used.yml")
  ))
  ...
  for (cell_type in key[, 1]) {
    ...
    PushToJobQueue(paste(
      "Rscript state_discovery_initial_plots.R",
      "discovery",
      discovery,
      fractions,
      cell_type,
...
if (!6 %in% skip_steps) {
  ...
  for (cell_type in key[, 1]) {
    ...
    n_clusters = key[key[, 1] == cell_type, 2]
    PushToJobQueue(paste(
      "Rscript state_discovery_first_filter.R",
      discovery,
      fractions,
      cell_type,
      n_clusters,
...
if (!7 %in% skip_steps) {
  ...
  PushToJobQueue(paste(
    "Rscript state_discovery_calculate_dropout_score.R",
    discovery,
    fractions
  ))
  RunJobQueue()
  ...
  for (cell_type in key[, 1]) {
    ...
    n_clusters = key[key[, 1] == cell_type, 2]
    PushToJobQueue(paste(
      "Rscript state_discovery_second_filter.R",
      discovery,
      fractions,
      cell_type,
      n_clusters,
...
if (!8 %in% skip_steps) {
  ...
  RunJobQueue()
  PushToJobQueue(paste(
    "Rscript ecotypes_assign_samples.R",
    discovery,
    fractions,
    "Ecotype",
    paste(additional_columns, collapse = " ")
