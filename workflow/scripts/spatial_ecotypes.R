...
if (all(paste0("CE", 1:10) %in% mapping$Ecotype)) {
  mapping$Ecotype = factor(
    as.character(mapping$Ecotype),
    levels = paste0("CE", 1:10)
  )
} else {
...
ecotype_data = do.call(
  ...
  lapply(splits, function(spl) {
    ...
    ecotypes = apply(ecotypes, 2, function(x) {
      x / quantile(ecotypes, .99, na.rm = T)
    })
...
melted = melt(
  as.data.frame(ecotype_data),
  id.vars = c("ID", "X", "Y", "Sample", "Malignant")
)
colnames(melted) = c(
  "ID",
  "X",
...
write.table(
  melted,
  file.path(output_dir, "ecotype_abundances_long.txt"),
  sep = "\t"
)
write.table(
  ecotype_data,
  file.path(output_dir, "ecotype_abundances.txt"),
  sep = "\t"
)
