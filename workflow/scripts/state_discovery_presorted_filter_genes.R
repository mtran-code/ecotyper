...
write.table(
  ...
  file.path(
    output_dir,
    paste0(to_test_cell_type, "_cell_type_specific_genes_raw.txt")
  ),
  sep = "\t",
  row.names = F
)
gene_info = gene_info[
  (gene_info$Q <= 0.05 & gene_info$FC > 0) |
    (is.na(gene_info$FC) & !is.na(df$Mean)),
]
write.table(
  gene_info,
  file.path(
    output_dir,
    paste0(to_test_cell_type, "_cell_type_specific_genes.txt")
  ),
  sep = "\t",
  row.names = F
)
