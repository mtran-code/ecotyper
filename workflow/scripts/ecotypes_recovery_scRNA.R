...
ecotype_discovery_dir = file.path(
  "../../data/results/ecotyper_work",
  dataset,
  fractions,
  "Ecotypes",
...
ecotypes = read.delim(
  file.path(ecotype_discovery_dir, 'ecotypes.txt'),
  sep = '\t'
)
mapping = read.delim(
  file.path(ecotype_discovery_dir, 'mapping_all_states.txt'),
  sep = '\t'
)
...
for (cell_type in key[, 1]) {
  ...
  classes_raw = classes_raw[
    classes_raw$InitialState %in% mapping$InitialState,
  ]
  classes_raw$State = mapping[
    match(classes_raw$InitialState, mapping$InitialState),
    "State"
  ]
  classes = classes_raw
...
filt = sort(unique(as.character(ecotypes[
  !ecotypes$CellType %in% colnames(all_classes_filt),
  "CellType"
])))
if (length(filt) > 0) {
  warning(paste0(
    "The following cell types are missing from the recovery cohort: ",
    paste(filt, collapse = ", "),
    ". Running ecotype recovery on the remaining ones!"
  ))
}
...
write.table(
  all_H,
  file.path(output_dir, "combined_state_abundances.txt"),
  sep = "\t"
)

H = do.call(
  rbind,
  lapply(levels(ecotypes$Ecotype), function(clst) {
    clst <<- clst
    #print(clst)
...
clinical$Ecotype = ifelse(
  clinical$AssignedToEcotypeStates,
  as.character(clinical$MaxEcotype),
  "Unassigned"
)
clinical$Ecotype = factor(
  as.character(clinical$Ecotype),
  levels = c(levels(ecotypes$Ecotype), "Unassigned")
)
    ...
    function(x) {
      ...
      if (sum(duplicated(tb[, 1])) > 0) {
        warning(paste0(
          "Not plotting column '",
          x,
          "', as it has multiple distinct values within the same sample!"
        ))
        return(NULL)
      }
...
write.table(
  clinical,
  file.path(output_dir, "initial_ecotype_assignment.txt"),
  sep = "\t"
)

clinical_filt = clinical[clinical$Ecotype != "Unassigned", ]
clinical_filt$Ecotype = factor(
  as.character(clinical_filt$Ecotype),
  levels = levels(ecotypes$Ecotype)
)
write.table(
  clinical_filt,
...
h <- heatmap_simple(
  all_H,
  top_annotation = clinical,
  top_columns = top_cols,
  left_annotation = ecotypes,
  left_columns = c("Ecotype", "CellType", "State"),
  column_split = ifelse(
    clinical$Ecotype == "Unassigned",
    "Unassigned",
    "Assigned"
  ),
  width = unit(7, "in"),
  ...
  color_range = seq(
    0,
    quantile(as.matrix(all_H), .9, na.rm = T),
    length.out = 8
  ),
...
decorate_heatmap_body("hmap", {
  grid.rect(
    ...
    hjust = 0,
    vjust = 1,
    gp = gpar(col = "white", fill = NA, lty = 1, lwd = 3)
  )
})
tmp = dev.off()
...
h = heatmap_simple(
  ...
  height = unit(3, "in"),
  legend_name = "State abundance",
  color_range = c(seq(0, quantile(small_H, .8, na.rm = T), length.out = 8)),
  color_palette = c("gray", brewer.pal(8, "YlGnBu")),
  raster_quality = 20
)

pdf(
  file.path(output_dir, "heatmap_assigned_samples_YlGnBu.pdf"),
  width = 8,
  height = 6
)
draw(
...
png(
  file.path(output_dir, "heatmap_assigned_samples_YlGnBu.png"),
  width = 8,
  height = 6,
  units = "in",
  res = 300
