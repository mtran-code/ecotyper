...
for (cell_type in cell_types[1:length(cell_types)]) {
  ...
  for (folder in folders) {
    #print(folder)
    files = list.files(
      file.path(input_dir, folder),
      pattern = paste0("CIBERSORTxHiRes_NA_", cell_type, "_Window*")
    )
    if (length(files) == 0) {
      stop(paste0(
        "No CIBERSORTxHiRes results generated by ",
        folder,
        " for cell type ",
        cell_type,
        "!"
  ...
  write.table(
    all_input,
    file.path(output_dir, paste0(cell_type, ".txt")),
    sep = "\t",
    row.names = F
  )
  write.table(
    dim(all_input),
    file.path(output_dir, paste0(cell_type, ".dimensions.txt")),
    sep = "\t",
    row.names = F
  )
}
