...
W = estim.r@fit@W
rownames(W) = c(
  paste0(rownames(raw_data), "__pos"),
  paste0(rownames(raw_data), "__neg")
)
...
classes = data.frame(
  ID = rownames(classes),
  State = sprintf("IS%02d", classes[, 1])
)
...
gene_info = get_FC_for_clusters(data, classes$State)
data = data[match(gene_info$Gene, rownames(data)), ]

write.table(
  gene_info,
  file.path(output_dir, "initial_gene_info.txt"),
  sep = "\t"
)
...
pdf(
  file.path(output_dir, "initial_assignment_heatmap.pdf"),
  width = 7,
  height = 6
)
draw(
...
tmp = dev.off()
png(
  file.path(output_dir, "initial_assignment_heatmap.png"),
  width = 7,
  height = 6,
  units = "in",
  res = 200
