...
tmp = apply(key, 1, function(x) {
  ...
  for (spl in splits) {
    df = data.frame(
      State = spl$State[1],
      CellType = cell_type,
      Dropout = mean(spl$Dropout),
      Expression = mean(spl$Expression)
    )
...
all_df$Dropout_index = scale(
  (as.vector(scale(all_df$Dropout)) + as.vector(scale(all_df$Expression)))
)
all_df$Dropout_index_discrete = as.factor(ifelse(
  all_df$Dropout_index < 1.96,
  "Non-outlier",
  "Outlier"
))
pdf(
  file.path(output_dir, "dropout_scores.pdf"),
  width = 5,
  height = 5,
  family = "Helvetica",
  useDingbats = F
...
g <- ggplot(all_df, aes(x = Dropout_index)) +
  ...
  geom_jitter(
    aes(y = 0.05, color = Dropout_index_discrete),
    height = 0.025,
    width = 0,
    size = 2
  ) +
...
png(
  file.path(output_dir, "dropout_scores.png"),
  width = 5,
  height = 5,
  units = "in",
  res = 400,
...
write.table(
  all_df,
  file.path(output_dir, "dropout_scores_with_outliers.txt"),
  sep = "\t"
)
